...
This document states the north star for **CAW-01**, the first visible product of the Company AI Workbench
and the heart of the system. It frames *why* the product exists and *what the first credible version proves*.
It does NOT specify UI mechanics, schemas, or build steps — those live in `05-*`, `04-*`, and `10-runbooks/`.
...
The same domain core is also reachable via **MCP** and **CLI** so other agents in the Company AI Workbench
substrate can drive it.
...
5. Preserve sources, assumptions, and outputs as an evidence row that can later feed a paper/patent.

## Why it matters (north star)

When this loop is credible, CAW-01 becomes the substrate for the rest of the program: paper/patent evidence
(CAW-03), trend/novelty radar (CAW-05), and future-AI/TTT workload axes (CAW-06) all plug into the same
experiment registry and trust ladder.
