...
A working data layer on SQLite kept Postgres-portable: the knowledge + simulation + HW-hierarchy + work-tree
tables, the repository implementations behind the core interfaces, and a filesystem artifact store.
...
3. **Do:** Create the **knowledge substrate** tables (Source, Claim, Evidence, …) with the **claim→evidence** constraint and `trust_level`/`boundary` columns.
