...
vertical slice. This ADR does **not** decide the work-tree *object model* (that is
...
- The data is **four substrates with different shapes** (SOURCE-BRIEF §9): a graph-ish **knowledge**
  substrate, a tabular **simulation** substrate (+ large trace blobs), a dense-graph **memory-annotated
  IR** (`MemoryAnnotatedIR/TensorNode/DataMovementEdge/FillLevel`), a containment-tree **HW design**
  substrate (chip→…→cluster), and a versioned **work-tree** substrate.
...
- **Provenance and the evidence chain are first-class** (SOURCE-BRIEF §1, §9 invariant, §11 guardrail):
  claims point to evidence; generated summaries are **not** evidence. This biases toward append-only,
  referentially-strong, auditable storage — a relational strength.
...
3. **Semantic search is `pgvector` inside the same Postgres** — added only when "find similar
   claims/runs/IRs" becomes a real user need. No second vector store at single-expert scale.
...
| `Source, Claim, Evidence, Note, Concept, Interest, OpenQuestion, Decision, Assumption` | **Postgres rows** (+ claim→evidence FK, `is_generated`) | enforce evidence invariant |
...
| Embeddings (text + run/IR feature vectors) | **pgvector** in same PG (deferred) | semantic recall without a second store |
...
1. **SQLite** (schema authored Postgres-portable via an ORM/migration tool targeting both) holds the
   relational core + knowledge rows + IR node/edge tables + HW adjacency tables + work-tree tables.
...
- **Hard / accepted:** deep IR/knowledge analytics are awkward in SQL until/unless AGE is added; the
...
- `TODO(open-question: measured latency threshold for IR/knowledge traversal that triggers Apache AGE)`
...
- **phase-0-foundations** — RB to stand up the SQLite (Postgres-portable) schema: relational core,
  knowledge rows with claim→evidence FK + `is_generated`, IR header + node/edge tables, HW adjacency
  tables, work-tree tables; plus the blob-on-FS path convention.
