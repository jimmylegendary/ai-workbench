# ADR-0001: Product surface — one shared core, three thin surfaces (web + MCP + CLI)
...
primary human surface, plus an MCP server and a CLI as automation surfaces, all sitting on **one shared
TypeScript core**. It also defines what a **"skill"** is in this product. It does **not** decide the
...
- **Brief inherited goal:** the workbench is part of the broader Company AI Workbench substrate and must be
  drivable by other agents — that is the MCP server's reason to exist.
...
A **skill** is a **packaged, named, declarative workflow** — a reusable recipe that composes core services
into a higher-order operation, with declared inputs/outputs and an **evidence contract** (trust-ladder
honesty per brief §1). A skill is **not** a new engine and **not** a new API; it is choreography over the
one core. The same skill is projected onto every surface:
...
- `TODO(open-question: ps-mcp-auth)` Does the MCP server need auth/multi-tenant scoping for the broader
  Company AI Workbench substrate, or is it single-trust local? (Revisit when CAW-01 is exposed beyond the
  local host.)
