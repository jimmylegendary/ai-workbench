...
이 제품은 *Company AI Workbench*(작업명: *Memory-Centric AI Workbench*)의 **CAW-01**입니다.
기존 프로그램 문서(`architecture.md`, `items/01-simulation-control-plane.md`, `TODO.md`)는 이미 다음을 확립했습니다:

- 이 워크벤치는 **솔버가 아니라 계측기(instrument)**입니다. 도메인 전문가가 설계 공간 축을 저렴하게 *이동/추가/테스트*하고,
...
- **신뢰 사다리(trust ladder)**가 신뢰성을 지배합니다: syntorch 트레이스는 A100/OTel 근거에 대해 검증되어야 하며,
  타일링/파티셔닝 가정은 산문이 아니라 명시적인 코드/전략 id여야 합니다.
- 설계 편향: **챗봇이 아니라 control plane처럼 느껴지게**(런 상태, 근거 완성도,
  열린 질문, 블로커, 아티팩트 준비도, 다음의 정직한 행동).

CAW-01은 첫 번째 가시적 제품이자 시스템의 핵심입니다. 이 브리프는 그 UI/UX와 엔진을 상세히 명세합니다.
...
- 동일한 백엔드/엔진은 **CLI** 및 **MCP server**로도 도달 가능해야 하며, 그래서 워크벤치를
  다른 에이전트나 더 넓은 Company AI Workbench 기반(substrate)에서 구동할 수 있어야 합니다.
  (웹 앱 = 주된 인간 표면; MCP/CLI = 자동화 표면; "skill" = 워크플로의 패키징.)
...
시스템은 최소한 다음을 저장하고 관계지어야 합니다:
- **지식 기반(Knowledge substrate)**: `Source, Claim, Evidence, Note, Concept, Interest, OpenQuestion, Decision, Assumption`
  (불변식: claim은 evidence를 가리킨다; 생성된 요약은 evidence가 아니다).
- **시뮬레이션 기반(Simulation substrate)**: `WorkloadModel, InputTrace, SimulationConfig, SimulationRun, TraceArtifact,
  Metric, ResultSet, ArchitectureProposal, MemoryProductRequirement, MemoryAnnotatedIR, TensorNode,
  DataMovementEdge, FillLevel`.
- **HW 설계 기반(substrate)**: chip/die/package/tray/rack/cluster 계층 + 컴포넌트 + 편집.
- **Work tree 기반(substrate)**: 세 캔버스 전반의 버전 관리된 변경 트리, 항목별 및 전체 저장 포함.
...
지식/HW/IR 그래프를 위한 **graph DB**(Neo4j), 그리고 **markdown 우선 / file 우선 DB**(git 추적
...
- ADR-0001 제품 표면 (웹 앱 주력 + MCP + CLI; 여기서 "skill"이란 무엇인가)
- ADR-0002 데이터 계층 (SQL / vector / Neo4j / md 우선 / 하이브리드)
