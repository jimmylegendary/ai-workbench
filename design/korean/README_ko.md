...
이 폴더는 **CAW-01, 즉 Simulation Control Plane(시뮬레이션 제어 평면)** 의 완전한 설계 + 빌드 명세서입니다. 이는
Company AI Workbench의 첫 번째 가시적 제품입니다. 이 문서는 **AI 빌더**에게 전달되도록 작성되었습니다. 설계 문서는
...
| `04` | [data-layer](./04-data-layer/) | 데이터 모델, 스토리지 전략, work-tree 스토리지, 지식 기반(knowledge substrate) |
...
**Next.js 웹 앱**(하나의 공유 `@caw/core` 위의 MCP + CLI 표면 포함)으로, 그 **Simulation** 화면은
...
- **표면(Surface):** 하나의 TS 코어 `@caw/core` + 얇은 web/MCP/CLI 표면.
