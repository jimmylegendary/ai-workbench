...
MCP 서버와 CLI가 웹 앱과 **동일한** `@caw/core` 연산을 어떻게 노출하는지, 그리고 Company AI Workbench에서
"skill"이 무엇을 의미하는지를 정의한다. 세 표면 모두 얇으며(thin), 계약은 [api-surface_ko.md](./api-surface_ko.md)이다.
...
이를 통해 기반 환경(substrate) 내의 다른 에이전트들이 워크벤치를 프로그램적으로 구동할 수 있다.
...
**skill**은 재사용 가능한 워크벤치 *워크플로*(명확한 입력/출력을 갖는 코어 연산의 시퀀스)를 패키징하여,
주로 MCP를 통해 노출함으로써 다른 에이전트들이 재사용할 수 있게 한다. skill은 계약 위의 조합(composition)이지
새로운 도메인 로직이 아니다.
...
- skill 패키징 포맷(manifest) — 더 넓은 Company AI Workbench skill 레지스트리와 정렬할 것; TODO(open-question).
