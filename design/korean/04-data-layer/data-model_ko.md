...
CAW-01의 논리적 스키마: 지식 기반(knowledge substrate), 시뮬레이션 기반(simulation substrate), HW 계층 구조, 그리고
graph-in-Postgres 접근 방식을 다룬다. 물리적 저장 위치(행 vs blob vs 벡터)는
[storage-strategy.md](./storage-strategy_ko.md)에, work-tree 테이블은 [work-tree-and-versioning.md](./work-tree-and-versioning_ko.md)에 있다.

## 규약

- 모든 테이블은 `id`(uuid), `created_at`, `created_by`, `surface`를 가진다.
- 외래 키는 명시적이며, **claim→evidence 불변식**이 강제된다(`Evidence`가 없는 `Claim`은 게시(publish) 대상으로 유효하지 않다).
- 그래프는 **adjacency/edge 테이블**로 저장되고 **recursive CTE**로 탐색된다(v1에서는 Neo4j 미사용, [ADR-0002](../01-decisions/ADR-0002-data-layer_ko.md)).

## 지식 기반(knowledge substrate)

| 엔티티 | 주요 컬럼 | 비고 |
| --- | --- | --- |
| `Source` | origin, date, author, license, retrieval_method, trust_level, boundary(public/internal/confidential) | 원천 입력 |
| `Claim` | statement, source_id, status | 게시 가능하려면 evidence를 가리켜야 함 |
| `Evidence` | claim_id, kind(run/source/measurement), ref (run_id 또는 source_id 또는 artifact 경로) | 증거 |
| `Note` / `Concept` / `Interest` / `OpenQuestion` / `Decision` / `Assumption` | text, links | 지식 그래프 노드 |

출처/신뢰(provenance/trust) 모델은 [knowledge-substrate.md](./knowledge-substrate_ko.md)를 참조하라.

## 시뮬레이션 기반(simulation substrate)
