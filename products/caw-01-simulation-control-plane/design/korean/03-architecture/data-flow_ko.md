...
## Flow C — 다른 제품으로의 증거/프로젝션 export

CAW-01은 자기 자신의 실행에 필요한 run-evidence/provenance만 보관한다. 다른 제품이 소비하는 것은 **독립 제품 간의 엄격한 export 경계**를 넘는다 — 공유 substrate, registry, DB는 없다. CAW-03(논문/특허 제품)은 이 export를 소비하는, 별도로 독립 배포되는 제품이다.
...
Experiment (refs) ─► EvidenceService.projection(refs[]) ─► Projection (comparable rows)
                  ...
                  ─► export artifact (claims point to evidence) ─► CAW-03 paper/patent (별도 제품)
```
