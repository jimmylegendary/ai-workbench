...
- **WorkloadModel / InputTrace / SimulationConfig / SimulationRun / TraceArtifact / Metric / ResultSet** — CAW-01 자체의 시뮬레이션 도메인 엔티티: 워크로드 정의, 그 입력 트레이스, 실행 구성, 실행 인스턴스, 생성된 트레이스 blob, 측정된 메트릭, 그리고 그룹화된 결과.
...
CAW-01은 독립적인 standalone 제품으로, 별도로 구현·배포되며 **공유 런타임 substrate가 없는** 6개 독립 제품군(CAW-01..06) 중 하나입니다. 아래 용어들은 CAW-01의 **자체** 내부를 설명합니다.

- **@caw/core** — CAW-01의 **자체 제품 코어**(TypeScript): 도메인 서비스(`ExperimentService`, `RunService`, `RegistryService`, `WorkTreeService`, `EvidenceService`) + Zod 계약; `next` 의존성 제로; 그 뒤에는 engine-adapter 포트와 리포지토리 인터페이스가 자리합니다. 다른 제품들이 공유하는 substrate가 아닙니다.
- **engine-adapter port** — 구체적 엔진(syntorch / LLMServingSim / ASTRA-sim)이 프로세스 외부(out-of-process)에서 구현하는 `@caw/core`의 인터페이스.
- **surface** — `@caw/core`로 향하는 얇은 진입점: 웹 앱(주력), MCP 서버, 또는 CLI. MCP/CLI는 외부 에이전트/도구가 **이** 제품을 구동할 수 있도록 하는 CAW-01 자체의 자동화 surface입니다.
- **skill** — **이 제품 자체의 연산(operation)**에 대한 재사용 가능한 워크플로의 패키징으로, (예: MCP를 통해) 다른 에이전트에게 노출됩니다.
- **export 경계(export boundary)** — CAW-01이 artifact(증거, projection, 요구사항)를 이를 소비할 수 있는 **다른 독립 제품**(예: CAW-02 지식 저장소, CAW-03 논문/특허 제품)에게 넘기는 경계. 독립 제품들 사이의 export이며 — 결코 공유 substrate, registry, 또는 데이터베이스가 아닙니다.
