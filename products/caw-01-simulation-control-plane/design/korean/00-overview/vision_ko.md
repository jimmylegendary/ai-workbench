...
이 문서는 독립적이고 독자적인 제품(separately implemented 및 deployed 되며 공유 런타임이 없는 6개 제품
제품군 CAW-01..06 중 하나)인 **CAW-01**의 북극성(north star)을 정의한다. 제품이 *왜* 존재하는지, 그리고
*첫 번째로 신뢰할 만한 버전이 무엇을 입증하는지*를 규정한다. UI 동작 방식, 스키마, 빌드 단계는 다루지
*않는다* — 그것들은 `05-*`, `04-*`, `10-runbooks/`에 있다.
...
동일한 제품 코어(`@caw/core`)는 **MCP**와 **CLI** — CAW-01 자체의 자동화 표면 — 를 통해서도 접근할 수 있어,
외부 에이전트와 도구가 *이* 제품을 구동할 수 있다.
...
5. 해당 실행 자체의 출처, 가정, 출력을 CAW-01이 나중에 별도의 논문/특허 제품으로 **내보낼(export)** 수 있는
   증거 행으로 보존한다.

## 왜 중요한가 (북극성)

이 루프가 신뢰할 만해지면, CAW-01은 메모리 중심 시뮬레이션 계측기로서 독자적으로 성립한다. 그 실행들이
보존되고 인용된 증거 행으로 끝나기 때문에, CAW-01은 깨끗한 제품 경계를 가로질러 *다른 독립 제품들* — 예:
논문/특허 제품(CAW-03) — 으로 아티팩트(증거, 비교 가능한 projection, 도출된 요구사항)를 **내보낼(export)** 수
있다. 그 제품들은 CAW-01의 내보내기를 소비할 뿐이며, CAW-01과 런타임, 레지스트리, 데이터베이스를 공유하지
않는다.
