...
CAW-01은 **독립적인 단독 제품(standalone product)**이다 — 6개의 분리된 제품군(CAW-01..06) 중 하나로, 각각 별도로
구현·배포되며 **공유 런타임 기반(shared runtime substrate)이 없다**. 이 문서는 **CAW-01이 노출하는 제품 표면이 몇
개인지, 각각이 무엇을 위한 것인지, 그리고 이들이 어떻게 CAW-01 자신의 제품 코어(product core) 위에 함께 놓이는지**를
결정하여, 웹 UI, MCP 서버, CLI가 결코 세 개의 서로 다른 제품으로 갈라지지 않도록 한다. 여기서 MCP와 CLI는 CAW-01의
**자체 자동화 표면**이다 — 외부 에이전트와 도구가 *이* 제품을 구동할 수 있도록 존재하며, 어떤 공유 워크벤치로의 진입점이
아니다. 계층화된 아키텍처(UI 표면 → 제품 코어/서비스 → 엔진 어댑터 → 데이터 계층), 역량(capability)→표면
...
CAW-01 자신의 자동화 표면; "skill" = 이 제품 자신의 연산에 대한 패키징된 워크플로.** 이 문서의 역할은 세 개의 백엔드를 만들지 않으면서 그것을 현실화하는 것이다.
...
| **MCP 서버** | CAW-01을 구동하려는 다른 에이전트/도구 (Claude 등) | 에이전트가 실험을 구성/실행하고, 트레이스/메트릭을 읽고, CAW-01 자신의 레지스트리를 **도구(tools) + 리소스(resources)**로 질의하도록 함 | 자체 검증 규칙을 가진, 두 번째로 갈라진 API |
...
### 4.1 MCP 서버 — 다른 에이전트를 위한 CAW-01 자신의 엔진/레지스트리

MCP TypeScript SDK는 세 가지 컨텍스트 유형을 노출한다; 우리는 CAW-01을 그것들에 직접 매핑한다. 이 MCP 서버는 *CAW-01 자신의* 연산을 노출하여 외부 에이전트가 이 제품을 구동하게 한다 — 어떤 공유 기반으로의 게이트웨이가 아니다:
...
| **Prompts** (재사용 가능한 템플릿) | 프롬프트 템플릿으로 노출된, CAW-01 자신의 연산에 대한 재사용 가능한 워크플로인 패키징된 **skills**(§4.3) | `skill: project-workload-to-memory-requirement`, `skill: compare-two-hardware-configs` |
...
### 4.3 CAW-01에서 "skill"의 의미

여기서 **skill**은 **이 제품 자신의 연산에 대한, 패키징된 이름 붙은 워크플로**이다 — 선언된 입력/출력과 증거 계약(evidence contract)을
가지고 CAW-01의 코어 서비스들을 고차(higher-order) 연산으로 조합하는 재사용 가능한 레시피이다. 그것은 새 엔진이 *아니고*
새 API도 *아니다*; 기존 코어 위의 안무(choreography)이다. (외부/공유 skill 레지스트리는 없다; skill은 CAW-01 자신의 것이다.)
...
- **OQ-PS-4:** 외부 에이전트/도구가 CAW-01의 MCP 서버를 구동할 때 인증/멀티테넌트 범위 지정이 필요한가, 아니면
