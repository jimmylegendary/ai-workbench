# ADR-0001: 제품 표면(Product surface) — 하나의 제품 코어, 세 개의 얇은 표면(web + MCP + CLI)
...
표면으로서의 Next.js 웹 앱, 그리고 자동화 표면으로서의 MCP 서버와 CLI이며, 이 모두가 **하나의 제품
코어**(`@caw/core`, CAW-01 자체의 TypeScript 코어) 위에 얹힌다. CAW-01은 **독립적이고 단독으로 동작하는
제품**이다 — 각각 별도로 구현·배포되고 그들 사이에 **공유 런타임 기반(substrate)이 없는** 6개 독립 제품
가족(CAW-01..06) 중 하나다. 또한 이 ADR은 이 제품에서 **"skill(스킬)"**이 무엇인지를 정의한다. 이 ADR은
...
- **Brief에서 상속된 목표:** CAW-01은 공유 런타임이 없는 단독 제품이지만, 외부 에이전트와 도구가 **이**
  제품을 구동할 수 있어야 한다 — 그것이 MCP 서버가 존재하는 이유다.
...
**skill**은 **이 제품 자체의 연산을 대상으로 하는, 패키징되고 이름이 붙은 선언적 워크플로** — 코어 서비스를
더 높은 차원의 연산으로 구성하는 재사용 가능한 레시피로서, 선언된 입력/출력과 **증거 계약**(brief §1의
trust-ladder 정직성)을 갖는다. skill은 새로운 엔진이 **아니며** 새로운 API도 **아니다**; CAW-01 자체 코어
위에서의 안무(choreography)다. 동일한 skill이 모든 표면에 투영된다:
...
- `TODO(open-question: ps-mcp-auth)` MCP 서버는 CAW-01이 로컬 호스트 너머로 노출될 때 인증/멀티테넌트
