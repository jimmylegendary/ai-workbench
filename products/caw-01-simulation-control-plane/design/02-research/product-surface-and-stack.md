...
CAW-01 is an **independent, standalone product** — one of a family of six separate products (CAW-01..06), each
separately implemented and deployed with **no shared runtime substrate**. This document decides **how many product
surfaces CAW-01 exposes, what each is for, and how they all sit on CAW-01's own product core** so that the web UI, an
MCP server, and a CLI never drift into three different products. The MCP and CLI here are CAW-01's **own automation
surfaces** — they exist so external agents and tools can drive *this* product — not an entry point into some shared
workbench. It defines the layered architecture (UI surfaces → product core/services → engine adapters → data layer),
a capability→surface matrix, and — critically — the **boundary between the TypeScript/Next.js side and the Python
simulation/trace side** (syntorch, LLMServingSim, ASTRA-sim, Chakra).
...
CAW-01's own automation surfaces; "skill" = a packaged workflow over this product's own operations.** The job of this
doc is to make that real without building three backends.
...
| **MCP server** | Other agents/tools (Claude, etc.) that want to drive CAW-01 | Letting an agent compose/run experiments, read traces/metrics, query CAW-01's own registry as **tools + resources** | A second, divergent API with its own validation rules |
...
│  @caw/core  — CAW-01's OWN product core: application services + domain types          │
...
└───────────────┬──────────────────────────────────────────────────────────────────────┘
                ▼
┌──────────────────────────────── DATA LAYER ─────────────────────────────────────────┐
│  Run-evidence/provenance · Simulation substrate · HW design substrate · Work-tree     │
...
### 4.1 MCP server — CAW-01's own engine/registry for other agents

The MCP TypeScript SDK exposes three context types; we map CAW-01 onto them directly. This MCP server exposes
*CAW-01's own* operations so an external agent can drive this product — it is not a gateway into any shared
substrate:
...
| **Prompts** (reusable templates) | Packaged **skills** — reusable workflows over CAW-01's own operations (§4.3) — surfaced as prompt templates | `skill: project-workload-to-memory-requirement`, `skill: compare-two-hardware-configs` |
...
### 4.3 What a "skill" means in CAW-01

A **skill** here is a **packaged, named workflow over this product's own operations** — a reusable recipe that
composes CAW-01's core services into a higher-order operation, with declared inputs/outputs and an evidence contract.
It is *not* a new engine and *not* a new API; it is choreography over the existing core. (There is no external/shared
skill registry; skills are CAW-01's own.)
...
- **OQ-PS-4:** Does CAW-01's MCP server need auth/multi-tenant scoping when external agents/tools drive it, or is it
  single-trust local? (MCP SDK has OAuth/URL-elicitation helpers if needed.) `TODO(open-question)`.
