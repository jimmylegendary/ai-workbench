...
| **P2** | **Trustworthy cost + real NPU twin + validation** | **in progress** | real NPU twins from public specs; cost with per-tier staging/placement + remainder cost + tightened reuse; **gate:** roofline lower-bound anchors pass AND cross-check vs ZigZag / a published Eyeriss/TPU number within a stated tolerance |
...
1. **P2 first** — if the numbers aren't trustworthy, everything above is meaningless.
   Real NPU twins (public specs) + a defensible cost model + a validation gate.
...
3. P4 → P6 after P2's gate passes.
